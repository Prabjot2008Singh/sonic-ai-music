...
const ChatPage: React.FC<ChatPageProps> = ({ theme, setTheme, userName }) => {
    ...
    const renderMainContent = () => {
        const renderContentWithBack = (content: React.ReactNode, title: string) => (
            <div className="flex flex-col h-full">
                <div className="flex items-center p-4 border-b border-gray-200 dark:border-gray-700 bg-white/50 dark:bg-black/20 backdrop-blur-sm">
                    <button
                        onClick={() => setMainContent('chat')}
        ...
        switch (mainContent) {
            ...
            case 'about':
                return renderContentWithBack(<AboutPage />, 'About');
            case 'terms':
                return renderContentWithBack(<TermsPage />, 'Terms of Service');
            case 'privacy':
                return renderContentWithBack(<PrivacyPage />, 'Privacy Policy');
            case 'contact':
                return renderContentWithBack(<ContactPage />, 'Contact Us');
            default:
                return null;
        }
    };
                ...
                <main className="flex-1 flex flex-col transition-all duration-300 overflow-hidden">
                    <div className="flex-grow flex flex-col p-2 sm:p-4 min-h-0">
                        <header className="flex-shrink-0 flex items-center mb-2 sm:mb-4 h-[44px]">
                            ...
                            {!isSidebarOpen && (
                                <h1
                                    className="text-xl font-bold text-white ml-4 cursor-pointer"
