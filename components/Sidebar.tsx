...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, queueCount, onOpenQueue, onOpenHistory, onOpenSettings, onNavigate }) => {
  ...
  return (
    <aside
      className={`bg-gray-100/80 dark:bg-gray-900/80 backdrop-blur-lg flex flex-col h-full transition-all duration-300 ease-in-out shadow-lg overflow-hidden ${isOpen ? 'w-64' : 'w-0'
        }`}
    >
      <div className="flex-shrink-0 p-4 flex items-center" style={{ height: '60px' }}>
        <div
          className={`flex items-center gap-3 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'} cursor-pointer`}
